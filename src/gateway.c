...
#include "nscache.h"
#endif // FEATURE_NSCACHE

const char gateway_h_rcs[] = GATEWAY_H_VERSION;

static jb_socket http_connect(const struct forward_spec * fwd,
...
void remember_connection(const struct reusable_connection *connection)
{
   ...
   assert(reusable_connection[slot].gateway_username == NULL);
   assert(reusable_connection[slot].gateway_password == NULL);
   ...
   if (NULL != connection->gateway_username)
   {
	   reusable_connection[slot].gateway_username = strdup_or_die(connection->gateway_username);
   }
   else
   {
	   reusable_connection[slot].gateway_username = NULL;
   }

   if (NULL != connection->gateway_password )
   {
	   reusable_connection[slot].gateway_password = strdup_or_die(connection->gateway_password);
   }
   else
   {
	   reusable_connection[slot].gateway_password = NULL;
   }
...
void mark_connection_closed(struct reusable_connection *closed_connection)
{
   ...
   freez(closed_connection->gateway_username);
   freez(closed_connection->gateway_password);
                            ...
                            struct client_state *csp)
{
   ...
   switch (fwd->type)
   {
      ...
      case FORWARD_WEBSERVER:
         ...
         break;
	  case SOCKS_HTTP:
		  sfd = http_connect(fwd, dest_host, dest_port, csp);
		  break;
      case SOCKS_4:
      case SOCKS_4A:
...
//HTTP代理发出的CONNECT请求
//仅支持Basic验证
#define HTTP_PROXY_CONNECT_WITH_AUTH "CONNECT %s:%d HTTP/1.1\r\n"\
	"Host: %s:%d\r\n"\
	"User-Agent: Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/43.0.2357.130 Safari/537.36\r\n"\
	"Proxy-Connection: Keep-Alive\r\n%s\r\n"
							//Basic验证头.
#define HTTP_PROXY_CONNECT_BASIC_AUTH "Proxy-Authorization: Basic %s\r\n"

static const char *get_basicauth_of_httpproxy_userpass(const struct forward_spec * fwd)
{
	char *base64_userpass;
	char szUserPass[500] = {0};

	if( fwd->gateway_username == NULL ) return NULL;

	snprintf( szUserPass,500, "%s:%s", fwd->gateway_username?fwd->gateway_username:"", fwd->gateway_password?fwd->gateway_password:"" );

	base64_userpass = base64_encode( (const unsigned char *)szUserPass,strlen( (const char *)szUserPass ) );
	if( base64_userpass  )
	{
		char szBasicAuthHeader[BUFFER_SIZE];
		snprintf( szBasicAuthHeader,BUFFER_SIZE,HTTP_PROXY_CONNECT_BASIC_AUTH,base64_userpass );
		free( base64_userpass );

		return strdup( szBasicAuthHeader );
	}
 ...
 *
 * Description :  Connect to the HTTP server, and connect through
                                ...
                                struct client_state *csp)
{
	...
	const char *final_socks_host;
#endif
	BOOL bNeedAuth = FALSE;
	const char *errstr = NULL;
	const char *basic_auth_userpass = NULL;

	log_error(LOG_LEVEL_CONNECT, "http_connect to target %s:%d via %s:%d", 
		target_host?target_host:"NA",target_port,fwd->gateway_host?fwd->gateway_host:"NA",fwd->gateway_port );
	...
	if (fwd->gateway_port <= 0)
   {
...
#ifdef FEATURE_NSCACHE
	// get ns from cache.
	final_socks_host = get_nscache( fwd->gateway_host );

	// can not find ns record from cache
	if( !final_socks_host )
	{
		// resolve proxy host to ip
		if ((addr = resolve_hostname_to_ip(fwd->gateway_host)) != INADDR_NONE)
		{
			inaddr.s_addr = addr;
			final_socks_host = strdup(inet_ntoa(inaddr));

			push_nscahce( fwd->gateway_host, final_socks_host );
		}
	}

	if( !final_socks_host )
	{
		errstr = "can not resolve socks server host name to ip address";
		err = 1;
	}
#endif
	...
	if (err)
	{
		errno = EINVAL;
		assert(errstr != NULL);
		log_error(LOG_LEVEL_CONNECT, "http_connect: %s", errstr);
		csp->error_message = strdup(errstr);
		freez( final_socks_host );

		return(JB_INVALID_SOCKET);
	}

	/* pass the request to the socks server */
	sfd = connect_to( final_socks_host, fwd->gateway_port, csp);

	if (sfd == JB_INVALID_SOCKET)
	{
		errstr = "http server unreachable";
		log_error(LOG_LEVEL_CONNECT, "http_connect: %s", errstr);
	...
	basic_auth_userpass = get_basicauth_of_httpproxy_userpass( fwd );
	snprintf( cbuf,BUFFER_SIZE,HTTP_PROXY_CONNECT_WITH_AUTH,
		target_host,target_port,
		target_host,target_port,
		basic_auth_userpass?basic_auth_userpass:""
		);
	freez( basic_auth_userpass );

	if (write_socket(sfd, cbuf, strlen( cbuf ) ))
	{
		errstr = "http request header write failed";
		csp->error_message = strdup(errstr);
	...
	if (!data_is_available(sfd, csp->config->socket_timeout))
	{
		...
		{
			errstr = "http request response wait timed out";
		}
		else
		{
			errstr = "http request response got aborted by the server";
		}
		err = 1;
	}
	...
	if( !err )
	{
		read_len = read_socket(sfd, sbuf, sizeof(sbuf));
		if (!err && read_len <= 0)
		{
			errstr = "http request response read failed";
			err = 1;
		}
	}
	...
	if( !err )
	{
		...
		first_endding = strstr( sbuf,"\r\n");
		if( !first_endding )
		{
			errstr = "http request response parse failed";
			err = 1;
		}
		else 
		{
			*first_endding = 0;
			first_empty = strchr( sbuf, ' ');
			if( first_empty )
			{
				int http_status = 0;
				char *http_status_des = NULL;

				first_empty ++;

				http_status = strtol(first_empty,&http_status_des,10 );
				...
				{
					errstr = "connect to http proxy failed.";
					err = 1;
				}
			}
			else {
                                ...
                                struct client_state *csp)
{
   ...
   log_error(LOG_LEVEL_CONNECT, "socks4_connect to target %s:%d via %s:%d", 
	   target_host?target_host:"NA",target_port,fwd->gateway_host?fwd->gateway_host:"NA",fwd->gateway_port );
   ...
   {
      case SOCKS_4:
		  // because SOCKS4 can not resolve host name.
   ...
   c->vn          = 4; // version
   c->cd          = 1; // connect
   c->dstport[0]  = (unsigned char)((target_port       >> 8 ) & 0xff); // destination port
   c->dstport[1]  = (unsigned char)((target_port            ) & 0xff); // destination port
   c->dstip[0]    = (unsigned char)((web_server_addr   >> 24) & 0xff); // destination ip
   c->dstip[1]    = (unsigned char)((web_server_addr   >> 16) & 0xff); // destination ip
   c->dstip[2]    = (unsigned char)((web_server_addr   >>  8) & 0xff); // destination ip
   c->dstip[3]    = (unsigned char)((web_server_addr        ) & 0xff); // destination ip
                                ...
                                struct client_state *csp)
{
...
#ifdef FEATURE_NSCACHE
   final_socks_host = NULL;
#endif

   log_error(LOG_LEVEL_CONNECT, "socks5_connect to target %s:%d via %s:%d", 
	   target_host?target_host:"NA",target_port,fwd->gateway_host?fwd->gateway_host:"NA",fwd->gateway_port );
   ...
   need_auth = FALSE;
   if( fwd->gateway_username != NULL )
	   need_auth = TRUE;

   client_pos = 0;
   cbuf[client_pos++] = '\x05'; /* Version */

   // socks server need auth?
   if( need_auth )
   {
	   cbuf[client_pos++] = '\x02'; /* Two authentication method supported */
	   cbuf[client_pos++] = '\x00'; // X'00' NO AUTHENTICATION REQUIRED
	   cbuf[client_pos++] = '\x02'; // X'02' USERNAME/PASSWORD
   }
   else 
   {
   ...
   if (!err && (sbuf[1] == '\xff'))
   {
      errstr = "none of the methods listed by the client are acceptable";
      err = 1;
   }
   ...
   { 
	   //Username/Password Authentication for SOCKS V5: http://tools.ietf.org/html/rfc1929
		...
		+----+------+----------+------+----------+
		*/
	   if( fwd->gateway_username && fwd->gateway_username[0] != 0 )
	   {
		   int len_un;
		   int len_pw;
		   int len_sent = 0;

		   len_un = (int)strlen(fwd->gateway_username);
		   len_pw = fwd->gateway_password?(int)strlen(fwd->gateway_password):0;

		   client_pos = 0;
		   cbuf[client_pos++] = '\x01'; /* Auth Version */
		   cbuf[client_pos++] = len_un;// length of username
		   memcpy( cbuf + client_pos , fwd->gateway_username, len_un ); // username
		   client_pos += len_un;
		   cbuf[client_pos++] = len_pw;// length of password
		   // 有些代理可能是有帐户名, 但是密码为空.
		   if( len_pw > 0 )
		   {
			   memcpy( cbuf+client_pos ,fwd->gateway_password, len_pw );
			   client_pos += len_pw;
		   }

		   if (write_socket(sfd, cbuf, client_pos))
		   {
			   errstr = "SOCKS5 authentication write failed";
			   csp->error_message = strdup(errstr);
			   log_error(LOG_LEVEL_CONNECT, "%s", errstr);
			   close_socket(sfd);
			   freez( final_socks_host );
		   ...
		   if (!data_is_available(sfd, csp->config->socket_timeout))
		   {
			   if (socket_is_still_alive(sfd))
			   {
				   errstr = "SOCKS5 authentication timed out";
			   }
			   else
			   {
				   errstr = "SOCKS5 authentication got aborted by the server";
			   }
			   err = 1;
		   }
		   ...
		   if (!err && read_socket(sfd, sbuf, sizeof(sbuf)) != 2)
		   {
			   errstr = "SOCKS5 authentication read failed";
			   err = 1;
		   }
		   ...
		   //	connection.
		   if( !err && ( sbuf[0] != 0x01 || sbuf[1] != 0x00) )
		   {
	   ...
	   {
		   errstr = "You haven't configed a username/password for socks proxy. but this socks server required a auth info";
		   err = 1;
	   }
   }
   ...
   cbuf[client_pos++] = '\x03'; /* Address is domain name, resolve domain name at remote SOCKS server*/
