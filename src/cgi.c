...
static const struct cgi_dispatcher cgi_dispatchers[] = {
   ...
   { "echo-pac",
	      cgi_send_pac,
		  "View the proxy-auto-config(PAC) script.",
		  TRUE },
#endif
...
struct http_response *dispatch_cgi(struct client_state *csp)
{
   ...
   /* Or it's the host part CGI_SITE_3_HOST, and the path CGI_SITE_3_PATH */
   else if( ( ( 0 == strcmpic(host, CGI_SITE_3_HOST) ) || (  (0 == strcmpic(host, CGI_SITE_2_HOST)) || (0 == strcmpic(host, CGI_SITE_2_HOST "."))))
	   && ( 0 == strncmpic(path, CGI_SITE_3_PATH, strlen(CGI_SITE_3_PATH)) )
	   )
   {
	   /* ..then the path will all be for us.  Remove leading '/' */
	   path++;
...
#ifdef RES_IN_DLL
/*
 * Read at most n-1 characters from the string memory.
 * Stop when a newline has been read, or the count runs out.
 * Return first argument, or NULL if no characters were read.
 */
char *mem_gets(char *buf, size_t n, const char **mem_buffer )
{
	int c;
	char *p;
	unsigned long len,copied;
	char * temp_buffer;

	if (n == 0 || mem_buffer == NULL || *mem_buffer == NULL )		/* sanity check */
		return (NULL);

	temp_buffer = *mem_buffer;
	...
	for (p = buf, n--; n > 0 && copied < len ; n--,copied++ ) 
	{
		*p++ = c = *(temp_buffer + copied);
	...
	*mem_buffer = temp_buffer + copied;

	return (p);
}

/************************************************************************
* Function : template_load_from_res
* 
* Description : Load templates from dll resource.
*
* Parameters  :
*          1  :  csp = Current client state (buffers, headers, etc...)
...
*		         JB_ERR_DLLMODULE if the dll module handle is none
*
************************************************************************/
jb_err template_load_from_res(const struct client_state *csp, char **template_ptr,
                     const char *templatename, int recursive)
{
   ...
   FILE *fp;
   char buf[BUFFER_SIZE];
   char *res_file_body,*temp_res_file_body;
   HRSRC hResource ;
   HGLOBAL hGlobal;
   void *pData ;
   unsigned long bytes;
   ...
   assert(template_ptr);
   assert(templatename);
   assert(g_hLibPrivoxyModule);

   *template_ptr = NULL;

   if( !g_hLibPrivoxyModule ) 
   {
	   log_error(LOG_LEVEL_ERROR, "DLL Module handle is NULL." );
	   return JB_ERR_DLLMODULE;
   }
   ...
   hResource = FindResource( g_hLibPrivoxyModule, templatename, TEMPLATES_RES_TYPE );
   if( !hResource )
   {
	   log_error(LOG_LEVEL_ERROR, "Find resource (%s) from dll error with code %d." ,TEMPLATES_RES_TYPE, GetLastError() );
		return JB_ERR_DLLRES;
   }

   hGlobal = LoadResource(g_hLibPrivoxyModule, hResource);
   if (!hGlobal)
   {
	   log_error(LOG_LEVEL_ERROR, "Load resource (%s) from dll error with code %d." ,TEMPLATES_RES_TYPE, GetLastError() );
	   return JB_ERR_DLLRES;
   }
   ...
   if( !pData )
   {
	   log_error(LOG_LEVEL_ERROR, "Lock resource (%s) from dll error with code %d." ,TEMPLATES_RES_TYPE, GetLastError() );
	   return JB_ERR_DLLRES;
   }

   bytes = SizeofResource(g_hLibPrivoxyModule, hResource);
   if( bytes <= 0 )
   {
	   log_error(LOG_LEVEL_ERROR, "Size resource (%s) from dll <= 0." ,TEMPLATES_RES_TYPE );
	   return JB_ERR_DLLRES;
   }
   ...
   if (file_buffer == NULL)
   {
	   log_error(LOG_LEVEL_ERROR, "Not enough free memory to buffer." );
	   return JB_ERR_MEMORY;
   }

   res_file_body = (char *)malloc( bytes + 1 );
   if( !res_file_body )
   {
	   log_error(LOG_LEVEL_ERROR, "Not enough free memory to buffer dll res file body." );
	   free( file_buffer );
	   return JB_ERR_DLLRES;
   }
   memcpy(res_file_body, pData, bytes );
   res_file_body[bytes] = 0;

   temp_res_file_body = res_file_body;
    ...
    */
   while (mem_gets(buf, sizeof(buf), &temp_res_file_body ))
   {
      if (!recursive && !strncmp(buf, "#include ", 9))
      {
         if (JB_ERR_OK != (err = template_load_from_res(csp, &included_module, chomp(buf + 9), 1)))
         {
            free(file_buffer);
			free( res_file_body );
            return err;
...
/*********************************************************************
 *
 * Function    :  template_fill_for_cgi_pac
 *
 * Description :  CGI support function that loads a pac script template
 *                and fills it in.  
 *
 * Parameters  :
 *          1  :  csp = Client state
 ...
 *
 *********************************************************************/
jb_err template_fill_for_cgi_pac(const struct client_state *csp,
                             const char *templatename,
                             struct map *exports,
                             struct http_response *rsp)
{
   jb_err err;
   size_t pac_file_size;
   struct stat statbuf[1];
   static time_t pac_file_lastmodified;
   static char *p_pac_file_body_cache;
...
#ifdef RES_IN_DLL
   // user specify pac file,the loaded from file directly.
   if( csp->config->pac_file )
   {
	   rsp->body = NULL;

	   if (stat(csp->config->pac_file, statbuf) >=0 )
	   {
		   if( pac_file_lastmodified == statbuf->st_mtime && p_pac_file_body_cache )
		   {
			   rsp->body = strdup( p_pac_file_body_cache );
		   }
		   else 
			   pac_file_lastmodified = statbuf->st_mtime ;
	   }
	   
	   if( rsp->body == NULL )
	   {
		   freez( p_pac_file_body_cache );
		   pac_file_size = 0;

		   err = load_file(csp->config->pac_file,&p_pac_file_body_cache,&pac_file_size);

		   if( err== JB_ERR_OK && p_pac_file_body_cache )
				rsp->body = strdup( p_pac_file_body_cache );
	   }
   }
   /** load pac template from res dll */
   else 
  {
	  rsp->body = NULL;

	  if( p_pac_file_body_cache )
	  {
		rsp->body = strdup( p_pac_file_body_cache );
		err = JB_ERR_OK;
	  }
	  ...
	  {
		  err = template_load_from_res(csp, &p_pac_file_body_cache, templatename, 0);
		  if( err == JB_ERR_OK && p_pac_file_body_cache )
		  {
			  rsp->body = strdup( p_pac_file_body_cache );
		  }
...
#endif
   if (err == JB_ERR_FILE || rsp->body == NULL )
   {
      free_map(exports);
   ...
   {
	   // 为了完整性和最佳的兼容性，
	   // 我们应该设置网页服务器(apache或者nginx,lighttpd等等)将这个pac文件的MIME类型
	   // 声明为 application/x-ns-proxy-autoconfig 或者 application/x-javascript-config
	   // 对于现代浏览器来说，两种mime类型都没有问题，　但 application/x-ns-proxy-autoconfig 
	   // 相对 application/x-javascript-config 被更多的客户端所支持,　因为它被定义在最初的Netscape规范里
		err = enlist_unique(rsp->headers, "Content-Type: application/x-ns-proxy-autoconfig", 13);
   }
...
#ifdef RES_IN_DLL
   err = template_load_from_res(csp, &rsp->body, templatename, 0);
#else
   err = template_load(csp, &rsp->body, templatename, 0);
#endif
   if (err == JB_ERR_FILE)
 ...
 *
 *********************************************************************/
struct map *pac_exports(const struct client_state *csp)
{
	jb_err err;
	struct map * exports;
	//char *user_extend_pac_file_body;
	//size_t user_extend_pac_file_size;
	//char *tempbuffer;
	char bufProxy[100];
	//struct stat statbuf[1];
	//static time_t pac_file_lastmodified;
	...
	//user_extend_pac_file_body = NULL;
	//user_extend_pac_file_size = 0;
	exports = new_map();

// 	if( csp->config->user_extend_pac_rule_file )
// 	{
// 		if (stat(csp->config->user_extend_pac_rule_file, statbuf) >=0 )
// 		{
// 			if( pac_file_lastmodified == statbuf->st_mtime && p_pac_file_body_cache )
// 			{
// 				user_extend_pac_file_body = strdup( p_pac_file_body_cache );
// 			}
// 			else 
// 				pac_file_lastmodified = statbuf->st_mtime ;
// 		}
// 
// 		if( user_extend_pac_file_body == NULL )
// 		{
// 			freez( p_pac_file_body_cache );
// 
// 			err = load_file(csp->config->user_extend_pac_rule_file,&p_pac_file_body_cache,&user_extend_pac_file_size);
// 
// 			if( err== JB_ERR_OK && p_pac_file_body_cache )
// 				user_extend_pac_file_body = strdup( p_pac_file_body_cache );
// 		}
// 	}
	...
	// has user extend pac file?
// 	if( user_extend_pac_file_body )
// 	{
// 		tempbuffer = implode_user_extend_pac_file( user_extend_pac_file_body );
// 		freez( user_extend_pac_file_body );
// 		user_extend_pac_file_body = tempbuffer;
// 	}

// 	if( user_extend_pac_file_body )
// 	{
// 		err = map(exports, "__USERRULE__", 1, user_extend_pac_file_body, 1 );
// 		freez( user_extend_pac_file_body );
// 	}
// 	else 
// 	{
		err = map(exports, "__USERRULE__", 1, "", 1 );
	//}

	sprintf(bufProxy,"PROXY %s:%d",CGI_SITE_3_HOST,csp->config->hport[0] );
	if( !err ) err = map(exports, "__PROXY__", 1, bufProxy, 1 );
