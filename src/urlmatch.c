...
jb_err parse_forwarder_address(char *address, char **hostname, int *port, char **username, char **password )
{
   ...
   *hostname = strdup_or_die(address);
   ...
   // 192.168.1.1:1080@base64(userpass)结构
   else if (NULL != (p = strchr(*hostname, ':')))
   {
      char *p1 = NULL;

      *p++ = '\0';
      *port = (int)strtol(p, &p1, 0);
	  ...
	  if( p1 !=NULL && *p1 == '@' )
	  {
		  ...
		  ret_len = 0;
		  base64_userpass =(char *) base64_decode( p1, &ret_len );
		  if( base64_userpass && ( p = strchr( base64_userpass ,':' )) != NULL )
		  {
			  *p++ = '\0';
			  if( username ) 
				  *username = strdup( base64_userpass );
