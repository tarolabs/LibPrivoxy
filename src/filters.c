...
static const struct forward_spec *get_forward_override_settings(struct client_state *csp)
{
   ...
   if ((vec_count == 2) && !stricmp(vec[0], "forward"))
   {
      fwd->type = SOCKS_NONE;
   ...
   else if ((vec_count == 2) && !stricmp(vec[0], "forward-webserver"))
   {
   ...
   else if (vec_count == 3)
   {
	  ...
	  if  (!stricmp(vec[0], "forward-http"))
	  {
		  fwd->type = SOCKS_HTTP;
		  socks_proxy = vec[1];
	  }
      else if  (!stricmp(vec[0], "forward-socks4"))
      {
         fwd->type = SOCKS_4;
         socks_proxy = vec[1];
      }
      else if (!stricmp(vec[0], "forward-socks4a"))
      {
         fwd->type = SOCKS_4A;
         socks_proxy = vec[1];
      }
      else if (!stricmp(vec[0], "forward-socks5"))
      {
         fwd->type = SOCKS_5;
         socks_proxy = vec[1];
      }
      else if (!stricmp(vec[0], "forward-socks5t"))
      {
         fwd->type = SOCKS_5T;
         socks_proxy = vec[1];
      }
      ...
      if (NULL != socks_proxy)
      {
         ...
         parse_forwarder_address(socks_proxy,
            &fwd->gateway_host, &fwd->gateway_port,&fwd->gateway_username, &fwd->gateway_password );
...
#ifdef FEATURE_ENABLE_FORWARD_OVERRIDE
