...
int g_terminate = 0;
	...
	extern char g_szDirecotryOfLibPrivoxy[1024];
#endif
                                 ...
                                 struct reusable_connection *server_connection)
{
   ...
   assert(server_connection->gateway_username == NULL);
   assert(server_connection->gateway_password == NULL);
   ...
   if( NULL != fwd->gateway_username )
   {
	   server_connection->gateway_username = strdup_or_die( fwd->gateway_username );
   }
   else 
   {
	   server_connection->gateway_username = NULL;
   }
   if( NULL != fwd->gateway_password )
   {
	   server_connection->gateway_password = strdup_or_die( fwd->gateway_password );
   }
   else 
   {
	   server_connection->gateway_password = NULL;
   }
 ...
 *********************************************************************/
#ifdef LIBPRIVOXY
static void privoxy_loop_serve(void *csp)
{
	listen_loop();
}

void do_privoxy_loop()
{
	_beginthread( (void (*)(void *))privoxy_loop_serve,0,NULL );
}
#endif
...
#ifdef LIBPRIVOXY
   static char path_of_privoxy[1024] = {0};
#endif
...
#ifdef LIBPRIVOXY 
   sprintf( path_of_privoxy,("%s\\privoxy.conf"), g_szDirecotryOfLibPrivoxy );
#endif
...
# endif /* def _WIN_CONSOLE */
   ...
   if (do_config_test)
   {
	   struct configuration_spec * load_config_result;
	   load_config_result = load_config();
#ifndef LIBPRIVOXY
	   if( load_config_result == NULL )
		   exit();
#endif
