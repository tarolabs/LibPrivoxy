...
#ifdef RES_IN_DLL
   err = template_load_from_res(csp, &rsp->body, "cgi-style.css", 0);
#else
...
#ifdef PAC_SCRIPT
/***********************************************************************
* Function : cgi_send_pac
*
* Description: send proxy-auto-config to web browser
*
* Parameters  :
*          1  :  csp = Current client state (buffers, headers, etc...)
*          2  :  rsp = http_response data structure for output
...
*
************************************************************************/
jb_err cgi_send_pac(struct client_state *csp,
				struct http_response *rsp,
				const struct map *parameters)
{
	struct map *exports;
	...
	if (NULL == (exports = pac_exports( csp )))
	{
		return JB_ERR_MEMORY;
	}

	return template_fill_for_cgi_pac(csp, "echo-pac", exports, rsp);
}
#endif
/*********************************************************************
 *
 ...
 *
 *********************************************************************/
jb_err load_file(const char *filename, char **buffer, size_t *length)
{
   FILE *fp;
   long ret;
   jb_err err = JB_ERR_OK;
