...
#define hash_forward_http				 2048343046U /* "forward-http */
...
#define hash_pac_file					   10619141U /* "pac-file" */
//#define hash_user_extend_pac_rule_file   3247152683U /* "user-extend-pac-rule-file" */
...
static void unload_configfile (void * data)
{
   ...
   while (cur_fwd != NULL)
   {
	  ...
	  freez(cur_fwd->gateway_username);
	  freez(cur_fwd->gateway_password);
      free(cur_fwd);
      cur_fwd = next_fwd;
   }
   config->forward = NULL;
...
#ifdef PAC_SCRIPT
   freez(config->pac_file);
   //freez(config->user_extend_pac_rule_file);
#endif
 ...
 * *************************************************************************/
         case hash_forward:
            ...
            if (strcmp(p, ".") != 0)
            {
               ...
               parse_forwarder_address(p, &cur_fwd->forward_host, &cur_fwd->forward_port,&cur_fwd->gateway_username,&cur_fwd->gateway_password );
            }
 ...
 * *************************************************************************/
         ...
         case hash_forward_socks5t:
			...
			if (directive_hash == hash_forward_http )
			{
				cur_fwd->type = SOCKS_HTTP;
			}
			else if (directive_hash == hash_forward_socks4)
			{
				cur_fwd->type = SOCKS_4;
			}
			else if (directive_hash == hash_forward_socks4a)
            {
			...
			//p = "nx1WSM9hJ9f8yxhOgf0SapcYRw==";

			cur_fwd->gateway_port = 1080;
#ifdef FEATURE_ENCRYPTCFG
			{
				char *DecryptedProxy = decrypt_msg( p, strlen( p ),TRUE );
				if( !DecryptedProxy )
				{
					log_error(LOG_LEVEL_ERROR,
						"Decrypt SOCKS proxy specifier for %s in configuration file.",
						cmd);
					string_append(&config->proxy_args,
						"<br>\nWARNING: Decrypt SOCKS proxy specifier for ");
					string_append(&config->proxy_args, cmd);
					string_append(&config->proxy_args,
						"directive in configuration file.");
					freez(cur_fwd);
				...
				parse_forwarder_address(DecryptedProxy, &cur_fwd->gateway_host,
					&cur_fwd->gateway_port,&cur_fwd->gateway_username,&cur_fwd->gateway_password );
...
#ifdef PAC_SCRIPT
		 case hash_pac_file:
			 if( stricmp( arg, "unset") != 0 )
			 {
				 freez(config->pac_file);
				config->pac_file = strdup(arg);
			 }
			 break;
// 		 case hash_user_extend_pac_rule_file:
// 			 if( stricmp( arg, "unset") != 0 )
// 			 {
// 				 freez(config->user_extend_pac_rule_file);
// 				config->user_extend_pac_rule_file = strdup(arg);
// 			 }
// 			 break;
#endif
/* *************************************************************************/
